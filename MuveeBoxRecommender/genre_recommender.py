...
class recommender():
	...
	def distance(self,genrescore1,genrescore2):
		sum_xy = 0
		sum_x = 0
		sum_y = 0
		sum_x2 = 0
	...
	def calGenreDistance(self):
		for user in self.trainGenreScore:
			scores = self.trainGenreScore
			dist = self.distance(self.testGenreScore[945], self.trainGenreScore[user])
			self.genreDistance[user] = dist
	
	def getRecommendations(self):
		self.topUsers = sorted(self.genreDistance, key = self.genreDistance.get, reverse = True)
		# pehle 5 ke 10, agle 10 ke 5
		count = 10
		for user in self.topUsers[:5]:
			count -= 10
			userRatings = self.traindata[user]
			topUserRatings = sorted(userRatings, key = userRatings.get)
			for movie in topUserRatings:
				if movie not in self.recommendations and not self.testdata[945].has_key(movie) and count < 10:
					self.recommendations.append(movie)
					count += 1
		for user in self.topUsers[5:10]:
			count -= 10
			userRatings = self.traindata[user]
			topUserRatings = sorted(userRatings, key = userRatings.get)
			for movie in topUserRatings:
	...
	def main(self):
		...
		self.calGenreDistance()
		#pprint(self.genreDistance)
